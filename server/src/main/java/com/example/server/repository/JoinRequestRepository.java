...
public interface JoinRequestRepository extends JpaRepository<JoinRequest, Integer> {
    ...
    @Query("""
            SELECT new com.example.server.model.dto.JoinRequestDTO(jr.id, jr.user.username, jr.project.id, jr.project.name,
    ...
    @Query("""
            SELECT new com.example.server.model.dto.JoinRequestDTO(jr.id, jr.project.id, jr.project.name, jr.project.owner.username,
             CASE WHEN jr.acceptFlag = true THEN 'accepted' WHEN jr.acceptFlag = false THEN 'rejected' ELSE 'waiting' END,
             jr.createdAt, jr.updatedAt)
